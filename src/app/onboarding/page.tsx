  ...
  if (!hydrated || !isAuthenticated) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-soft px-4">
        <p className="rounded-2xl bg-white px-5 py-4 text-sm font-bold text-slate-600 shadow-soft">Memeriksa sesi login...</p>
      </main>
    );
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-soft px-4 py-8">
      <Card className="w-full max-w-xl">
        ...
        <div className="mt-6">
          ...
          {step === 4 && <><h1 className="text-2xl font-black text-slate-900">Mode AI otomatis</h1><p className="mt-2 text-sm text-slate-500">AI membantu memberi ringkasan, langkah, dan rekomendasi jadwal. Semua hasil tetap bisa diedit.</p><button onClick={() => setAiEnabled(!aiEnabled)} className="mt-5 w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left font-bold text-slate-800">{aiEnabled ? "✅ AI Recommendation aktif" : "⬜ AI Recommendation nonaktif"}</button></>}
        </div>
